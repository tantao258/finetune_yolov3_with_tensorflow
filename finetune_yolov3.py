...
with tf.Session() as sess:
    ...
    loss_summary = tf.summary.scalar("loss", sum(yolov3.loss))
    # acc_summary = tf.summary.scalar("accuracy", yolov3.accuracy)

    # merge all the train summary
    train_summary_merged = tf.summary.merge([loss_summary, grad_summaries_merged])
    ...
    val_summary_merged = tf.summary.merge([loss_summary])
    ...
    while True:
        ...
        _, step, train_summaries, loss = sess.run([yolov3.train_op, yolov3.global_step, train_summary_merged, yolov3.loss],
                                                  feed_dict={
                                                        yolov3.x_input: image,
